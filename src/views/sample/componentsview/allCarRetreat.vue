<template>
    <!-- 整车-退样列表 -->
    <div class="allCarRetreat">
        <div class="allCarRetreat_con">
            <!-- 查询 -->
                    ...
                    <el-form :inline="true" :model="queryForm" class="query_form" label-width="80px">
                        <el-form-item label="送样单位：" >
                            <el-input clearable name="clearInputValue" @keyup.enter.native="onPressEnter" 
                                placeholder="请输入送样单位" v-model.trim="queryForm.comeCompany">
                            </el-input>
                        </el-form-item>
                        <el-form-item label="送样人：">
                            <el-input clearable name="clearInputValue" @keyup.enter.native="onPressEnter" 
                                placeholder="请输入送样人" v-model.trim="queryForm.comePeople">
                            </el-input>
                        </el-form-item>
                        <el-form-item label="收样人：">
                            <el-input clearable name="clearInputValue" @keyup.enter.native="onPressEnter" 
                                placeholder="请输入收样人" v-model.trim="queryForm.incomePeople">
                            </el-input>
                        </el-form-item>
                        <el-form-item label="经办人：">
                            <el-input clearable name="clearInputValue" @keyup.enter.native="onPressEnter" 
                                placeholder="请输入经办人" v-model.trim="queryForm.operator">
                            </el-input>
                        </el-form-item>
                        <el-form-item label="任务号：" v-show="showState">
                            <el-input clearable name="clearInputValue" @keyup.enter.native="onPressEnter" 
                                placeholder="请输入任务号" v-model.trim="queryForm.taskCode">
                            </el-input>
                        </el-form-item>
                        <el-form-item label="取样人：" v-show="showState">
                            <el-input clearable name="clearInputValue" @keyup.enter.native="onPressEnter" 
                                placeholder="请输入取样人" v-model.trim="queryForm.outPeople">
                            </el-input>
                        </el-form-item>
                        <el-form-item label="VIN：" v-show="showState">
                            <el-input clearable name="clearInputValue" @keyup.enter.native="onPressEnter" 
                                placeholder="请输入VIN" v-model.trim="queryForm.vin">
                            </el-input>
                        </el-form-item>
                        <el-form-item label="车型：" v-show="showState">
                            <el-input clearable name="clearInputValue" @keyup.enter.native="onPressEnter"
                                placeholder="请输入车型" v-model.trim="queryForm.model">
                            </el-input>
                        </el-form-item>
                        <el-form-item label="退样日期：" v-show="showState">
                            <el-date-picker
                                v-model="queryForm.outDate"
                                type="daterange"
                                range-separator="至"
            ...
            <div class="tablePagination_modular" v-loading="loading" >
                ...
                <div class="table_modular">
                    <el-table ref="table" border stripe :data="tableData"
                        ...
                        <el-table-column align="center" :resizable="false" prop="outDate" label="退样日期"></el-table-column>
                        ...
                        <el-table-column align="center" :resizable="false" prop="outPeople" label="取样人"></el-table-column>
                        <el-table-column align="center" :resizable="false" prop="operator" label="经办人"></el-table-column>
                        <el-table-column align="center" :resizable="false" prop="status" label="状态" width="160">
                        ...
                        <el-table-column align="center" :resizable="false" label="操作" width="260">
                            <template slot-scope="scope">
                                ...
                                <el-button type="text" >WORD</el-button>
        ...
        <!-- 流转 -->
        <el-dialog title="流转" width="80%" class="dialog_flowTurn" 
...
export default {
    ...
    methods: {
        ...
        onQuery(current, pageSize) {
            ...
            this.$api.vehicle.queryData(params).then(res => {
        ...
        onCancelRetreat(row) {
            this.$confirm('确认要取消退样吗？', '提示', {
                ...
                console.log('确定');
                this.$api.vehicle.cancelCarBack({terminalid: row.id}).then(res => {
        ...
        batchRetreatKind() {
            if(this.multipleSelection.length > 0) {
                ...
                idList = idList.substring(0, idList.length-1);
                this.$api.vehicle.batchRetreatKind({idList}).then(res => {
                    // console.log(res, '批量退样成功');
                    this.$message.success('批量退样成功！');
                    this.onQuery(this.current, this.pageSize);
                }).catch((error) => {
                    console.log(error, '批量退样失败');
                    this.$message.error('批量退样失败！');
                    this.onQuery(this.current, this.pageSize);
                });
            } else {
                this.$message.warning('请先选择整车信息，再进行该操作！')
        ...
        batchCancelRetreat() {
            if(this.multipleSelection.length > 0) {
                ...
                idList = idList.substring(0, idList.length-1);
                this.$api.vehicle.batchCancelRetreat({idList}).then(res => {

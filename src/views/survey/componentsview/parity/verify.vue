  ...
  <section class="dislogBox" :style="{
            ...
            <div class="oneFirstFloor" id="oneFirstFloor" v-if="item.ckenck == true">
              ...
              <div class="pot">
                ...
                <ul class="cB_listTitle">
                  ...
                  <li class="trademark2_4">报告审核人</li>
                    ...
                    <ul class="onecB_listcenter" :style="flowColor(item.flowStatus)">
                      <li class="tasknumber2">
                        <el-checkbox v-model=item.checked :label="item" @change="inspectids(item)"
                          v-if="item.process_instance_id != 4 || item.isCurrentPower  == 0 ? false : true"
                      ...
                      <!-- <li class="trademark2_1" :title=item.testmethod>{{item.testmethod}}</li> -->
                      ...
                      <li class="trademark2_4">
                        ...
                        <!-- <el-select v-model="item.inspecopval" placeholder="请选择" class="incolde selwidth"
                                   v-if="item.process_instance_id != 4 ? false : true"
                      ...
                      <li class="trademark2_3">
                        <span v-if="item.process_instance_id != 4 || item.isCurrentPower  == 0 ? false : true">{{item.cachetType}}</span>
                      ...
                      <li class="operation2">
                        ...
                        <i>&nbsp;</i>
                        <span class="spanclass" @click="look(item)"
                              v-if="item.needReport == '1' ? true : false">查看报告</span>
                        <i>&nbsp;</i>
    ...
    <el-dialog title="报告生成" :visible.sync="dialogFormVisible"
      ...
      <section class="upData">
        ...
        <div>
          报告自动填充：
          <el-button type="primary">报告自动生成</el-button>
        </div>
        <p>方式2：</p>
        <div class="pszet" v-if="falg">
          ...
          <el-upload
            ...
            ref="upload"
            action="1"
            style="width:28%;float: left; margin-left: 10px; position: relative;"
      ...
      <div class="mybox">
        <!-- <ul>
          <li v-for="(item,index) in regList" :key='index'>
            ...
            <span class="cl2"><i @click="lookPresentation(item)">{{item.fileName}}.{{item.fileType}}</i><b
              @click="delitem(item.fileId)">×</b></span>
      ...
      * */
        ...
        this.$api.lxtinitial.getwomuntow(this.valTwomun.mid, data).then(res => {
          ...
          if (data.length == 0) {
            ...
            data.forEach((item, index) => {
              ...
              this.inspectBox.push(item.shenHeList) //报告审核人下拉列表
              let content
              let obj
              let opval = '' //下拉框默认值
              ...
              obj = {
                ...
                cachetType: item.cachetType, // 图章回显字段
        ...
        if (val) {
          ...
          arrTowList.forEach((item, index) => {
            ...
            if (reg.test(item.inspecopval)) {//?????
             ...
             if(item.process_instance_id == 4 && item.isCurrentPower != 0){
      ...
      inspectids (item) { //选中的数据
        ...
        if (item.checked == false) {
          ...
          this.inBox.push({id: item.id, value: item.cachetType, value1: item.valueId})
      ...
      gotoNext (tofinish) { //提交下一步  直接完成
        ...
        if (_list1.length == 0) {
          ...
          this.$alert('请先确认选择了当前选中的报告校核人', '信息提示', {
            dangerouslyUseHTMLString: true
          })
        }
      },

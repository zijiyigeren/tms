                ...
                <el-checkbox-group v-model="checkedCities" @change="handleCheckedCitiesChange" class="bordeLine">
                  <div class="cB_theCirculation" v-for="(item,index) in dataListTwo" :key="item.id">
                    <ul class="onecB_listcenter" :style="flowColor(item.flowStatus)">
                      ...
                      <!-- <li class="trademark2_1" :title=item.testmethod>
      ...
      * */
      ...
      handleCheckAllChange(val) {  //全选
        ...
        if (val) {
          ...
          arrTowList.forEach((item, index) => {
            ...
            if(item.process_instance_id == 3){
              let obj = {"id": item.id, "value": item.selvalue,"value1":item.inspecopval};//?????
              arr.push(obj)
            }
      ...
      inspectids(item) { //选中的数据
        ...
        if (item.checked == false) {
          ...
          this.inBox.push({id: item.id, value: item.selvalue, value1:item.inspecopval});
...
<style lang="scss" scoped>
  ...
  // 新增
  .mybox{ width: 100%; height: auto; overflow: hidden;}
  .mybox ul{ margin: 0; padding: 0;}
  .mybox ul li{width: 100%; height: 40px; line-height: 40px; overflow: hidden;}
  .mybox ul li span{display: block; float: left; }
  .mybox ul li span.cl1{  width: 15%;}
  .mybox ul li span.cl2{  width: 60%; border-bottom: 1px #ccc solid; height: 38px;  position: relative;}
  .mybox ul li span.cl2 i{font-style:normal}
  .mybox ul li span.cl2 b{ position: absolute; right: 0; color: #ccc; font-size: 18px !important; cursor: pointer;}
  .mybox ul li span.cl3{ width: 15%; text-align: center; height: 28px; line-height: 28px; margin-top: 8px; border: 1px #ccc solid; margin-left: 2%; cursor: pointer;}

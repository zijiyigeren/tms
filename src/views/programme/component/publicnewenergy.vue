<template>
    <!-- 上传公共组件 -->
  <div class="lxt_box_new">
    <div class="fromBox_new" id="fromBox_new">
      <div class="boxtitle">
        <el-button type="primary" size="small" @click="refresh_new">刷新</el-button>
        <el-button type="success" size="small" @click="preservation_new">保存</el-button>
        <el-button type="info" size="small" @click="reportDirectory_new">上送报告目录</el-button>
        <el-button type="warning" size="small" @click="submitReportFile_new">上送报告文件</el-button>
      ...
      <div class="fromtop_new" id="fromtop_new">
        <table width="100%" border="0" bordercolor="#EBEEF5"   cellspacing="0" id="fromdata_new" style="border-collapse:separate; border-spacing:0px 5px;">
            <tr class="trForm"  >
              <td width="10%">序号</td>
...
export default {
    ...
    data() {
      return {
          ...
          // },
          alertcoim : window.goReplace_new =(id)=>{  
            this.$confirm('是否替换?', '提示', {
              confirmButtonText: '确定',
              cancelButtonText: '取消',
              type: 'warning'
            }).then(() => {
              let oAllDataBox = document.getElementById('fromdata_new');
          ...
          winAll : window.allCheck_new = (category) =>{ //全选
              let ofromdata_new = document.getElementById('fromdata_new');
              let ainput = ofromdata_new.getElementsByTagName('input')
              if(category == '△'){ //三角
                if(sanjiaoFale){
          ...
          updateState : window.updateState =()=>{  // 每次加载都需要遍历 复选框状态
              var ofromdata_new = document.getElementById('fromdata_new');
              var ainput = ofromdata_new.getElementsByTagName('input');
              if(sanjiaoFale){  //三角
                for(let i=0; i<ainput.length; i++){
                    if(ainput[i].name == 'itemsid'){
                      ainput[i].checked = true;
    ...
    methods: {
      ...
      allData(val){ //获取数据
       ...
       let dataList = res.data.data.ndetail.list
      ...
      alltable(){ //渲染数据
        ...
        let ofromtop_new = document.getElementById('fromdata_new');
        numfrequency ++;
        for(let i=0; i<allDatas.length; i++){
          if(i >= numpositions){
            ...
            if(allDatas[i].category =='△'){ 
                ...
                if(sanjiao == 1){
                  ...
                  odiv.innerHTML += `<tr class="trForma" >
                      <td>全选
                        <input type="checkbox" name="sbox2" value="" checked onchange="allCheck_new('${allDatas[i].category}')"/> 
                        <input type="checkbox" name="itemsid" value="" checked data-id = "${allDatas[i].seritel}" /> 
                        <span >${allDatas[i].serial}</span>
                        ${allDatas[i].seritel}
                      </td>
                      <td>${allDatas[i].name}</td>
                      <td class="catDack">${allDatas[i].category}</td>
                      <td>${allDatas[i].number}</td>
                      <td>${allDatas[i].standard}</td>
                      <td><textarea rows="3" class="outline">${allDatas[i].remarks}</textarea></td>
                      <td><textarea rows="3" class="outline">${allDatas[i].deemed}</textarea></td>
                      <td><textarea rows="3" class="outline">${allDatas[i].measured}</textarea></td>
                      <td><textarea rows="3" class="outline">${allDatas[i].similar}</textarea></td>
                      <td><textarea rows="1" class="outline">${allDatas[i].videofile}</textarea>
                       <textarea rows="1" class="outline">${allDatas[i].incuke}</textarea>
                        
                      </td>
                      <td>
                        <span style="word-warp:break-word;word-break:break-all"></span>
                      </td>
                      <td><textarea rows="3" class="outline">${allDatas[i].matching}</textarea></td>
                  </tr>`
                }else{
                  odiv.innerHTML += `<tr class="trForma" >
                      <td>
              ...
              if(allDatas[i].category =='△○'){ 
                ...
                if(source == 1){
                  odiv.innerHTML += `<tr class="trForma" >
                      <td>全选
                        <input type="checkbox" name="sbox2" value="" onchange="allCheck_new('${allDatas[i].category}')"/> 
           ...
           ofromtop_new.appendChild(odiv)
        }
      },
      rollerEvent(){ //判断滚动条是否触底
        let clientHeight = document.getElementById('fromBox_new').offsetHeight;// 屏幕高度  
        let scrollObj = document.getElementById('fromtop_new'); // 滚动区域
      ...
      preservation_new(){ //保存按钮
        ...
        let oTr = oAllDataBox.getElementsByTagName('tr')
        for(let i=1; i<aAllInput.length; i++){
      ...
      refresh_new(){ //刷新
        ...
        this.$api.tatistical.refreshPreservation(Data).then((res)=>{
                      ...
                      if (_measured.value.trim().length<=0){
                         ...
                         _measured.value=item.itemscode;
                          ...
                          if (_measured.value==item.itemscode){
                            ...
                            if (isErrQA)
                            {
                             _stateOne.innerHTML=`<a href="javascript:goReplace_new('${item.id}');"><font color=red>报告没有下达 错误请对方案</font></a>`;
                            }
                            else
                            {
                              if (item.itemscode=="")
                              ...
                              }else  _stateOne.innerHTML=`<a href="javascript:goReplace_new('${item.id}');"><font color=red>替换</font></a>`;
                            }
                        ...
                        {
                          _matchingResults.value=item.itemscode;
                          _stateOne.innerHTML=`<a href="javascript:goReplace_new('${item.id}');">替换</a>`;
                        }
                        else
                        {
                          ...
                          else
                          {
                            if (pvalue == item.itemscode)
                            {
                              ...
                              _stateOne.innerHTML=`<a href="javascript:goReplace_new('${item.id}');"><font color=red>替换</font></a>`;
    ...
    mounted() {
      document.getElementById('fromtop_new').addEventListener('scroll', this.rollerEvent, true);  // 监听（绑定）滚轮滚动事件
    },
    destroyed () {
      document.getElementById('fromtop_new').removeEventListener('scroll', this.rollerEvent);   //  离开页面清除（移除）滚轮滚动事件
    },
    watch:{
      resData(val){

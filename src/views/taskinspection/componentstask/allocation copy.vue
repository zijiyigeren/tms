  ...
  export default {
    ...
    data() {
      return {
        ...
        // 选项列表（必选）
        list: [
          // { id: 1, parentId: 0, org_name: "一级菜单A", rank: 1 },
          // { id: 2, parentId: 0, org_name: "一级菜单B", rank: 1 },
          // { id: 3, parentId: 0, org_name: "一级菜单C", rank: 1 },
          // { id: 4, parentId: 1, org_name: "二级菜单A-A", rank: 2 },
          // { id: 5, parentId: 1, org_name: "二级菜单A-B", rank: 2 },
          // { id: 6, parentId: 2, org_name: "二级菜单B-A", rank: 2 },
          // { id: 7, parentId: 4, org_name: "三级菜单A-A-A", rank: 3 },
          // { id: 8, parentId: 7, org_name: "四级菜单A-A-A-A", rank: 4 },
          // { id: 9, parentId: 0, org_name: "一级菜单C", rank: 1 },
          // { id: 10, parentId: 0, org_name: "一级菜单end", rank: 1 }
        ],
    ...
    methods: {
      ...
      * */
      ...
      cueeedtin(id,teg,isliu,index){
         let bbw = []
         console.log(id)
        this.$api.allot.findOfficedepart(id,this.carType).then(res => {
            if(res.data.data){
      ...
      * */
      ...
      toogleExpand(row) {  // 第一个弹出层
        ...
        this.carType = row.carType //更新 报告编辑人请求数据的参数
        this.multi = []
        this.loading2 = true;
        let obj = {
          companyCheck: this.formLabelAlign.unit,
        ...
        this.$api.allot.findOfficeMode().then(res => { //获取科室数据
      ...
      obss(obj) {
        this.$api.allot.findJobContent(obj).then(res => {
          ...
          this.tableData2.map((item,index) => {
            ...
            this.valueId = item.testRoom

            this.offitem = item.itemsRoom
            ...
            this.$set(item, 'findOffice', this.findOffice)
            this.$set(item, 'offitem',  this.offitem)
      ...
      batchBtn() {  //显示弹出层
        ...
        this.$api.allot.findOfficeMode().then(res => { //获取科室数据
          console.log(res)
          this.bgfindOffice = res.data.data;
          // this.findOfficeitem = res.data.data[0].id
          this.bgDepartment = this.bgfindOffice[0].id
          return this.bgfindOffice[0].id;
        }).then((res)=>{
           this.cueeedtin(res)
           this.batchdelg = true;
        }).catch(err => {
      ...
      batchdelgFn(){ //批量分配掉接口
        ...
        console.log(str)
        console.log(this.bgDepartment)
        console.log(this.bgPeople)

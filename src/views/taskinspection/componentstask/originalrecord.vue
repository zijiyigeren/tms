    ...
    <div class="allot" :style="{
          ...
          <el-form :inline="true" :model="formLabelAlign"
            ...
            <div style="display: flex;justify-content: space-between;">
              ...
              <el-form-item style="width:19%">
                ...
                <el-input v-model="formLabelAlign.trademark" @keyup.enter.native="searchEnterFun"
                          name="clearInputValue"></el-input>
              </el-form-item>
            </div>
          </el-form>
              ...
              <template slot-scope="scope">
                ...
                <div>
                  <el-table ref="Table2" border v-loading="loading2"
                    ...
                    <el-table-column prop="itemsFrederickItme" label="检验方法" align="center"
                                     :resizable="false"></el-table-column>
                    ...
                    <el-table-column prop="testPersonName" label="检验员" align="center"
                                     :resizable="false"></el-table-column>

                    <el-table-column prop="assignee" label="校核人员" align="center" :resizable="false">
                      <template slot-scope="scope">
                        <el-select
                          size="mini"
                          style="width:80%;"
                          ...
                          <el-option v-for="(item,i) in tableData2[scope.$index].yuanShiJiLuShenHeList" :key="i"
                    ...
                    <el-table-column prop="orgName" label="报告编辑科室" align="center" :resizable="false">
                      <template slot-scope="scope">
                        <span v-show="scope.row.needReport == 1 ? true : false">{{tableData2[scope.$index].orgName}}</span>
                      </template>
                    ...
                    <el-table-column prop="checkTypeID" label="操作" align="center" style="color:#32AFF0;"
                                     :resizable="false">
                      <template slot-scope="scope">
                        <el-button type="text" style="color:#32AFF0;marginLeft:6px;"
                        ...
                        <el-button type="text" style="color:#32AFF0;marginLeft:6px;"
                                   @click="clickOriginalrecord(scope.row)">
                          编辑原始记录
                        </el-button>
                      </template>
                    </el-table-column>
                      ...
                      <template>
                        ...
                        <el-table border :data="tableData3" v-loading="loading3" style="width: 100%"
                          ...
                          <el-table-column align="center" label="节点名称" :resizable="false">
                            <template slot-scope="scope">
                              <span
                                :class="tableData3[scope.$index].reason != null && tableData3[scope.$index].reason.length > 0 ? 'active' : ''">{{scope.row.flowname}}</span>
                            </template>
                          </el-table-column>
      ...
      <el-dialog
        ...
        <div>
          ...
          <div class="recordbox">
            <div class="left">手记原始记录：</div>
            <div class="right">
              <el-upload
                action="1" ref="upload"  multiple
                class="upload-demo"
        ...
        <!-- <span slot="footer" class="dialog-footer">
          <el-button @click="updatedok">取 消</el-button>
          <el-button type="primary" @click="updatedok">确 定</el-button>
        </span> -->
      </el-dialog>
      ...
      <el-dialog
        title="原始记录上传"
        :visible.sync="popupisible"
        width="70%"
        >
        <div style="margin-bottom:10px">
         <el-button type="primary" class="bespoke" @click="onlineediting(true)">在线编辑</el-button>
         <el-button type="primary" class="bespoke" @click="importhistory(true)">导入历史原始记录</el-button>
         <el-button type="primary" class="bespoke" @click="localupload">本地上传</el-button>
        </div>
        ...
        <ul ref="myboxList" >
          ...
          <li v-for="(item,index) in singleTable" :key='index'>
            ...
            <span class="yoperation">
              ...
              <el-button
                v-if="item.fileType == 2"
                type="text" style="color:#32AFF0;marginLeft:6px;"
                @click="handleLook(index, item)">查看</el-button>
              <el-button
                type="text" style="color:#32AFF0;marginLeft:6px;"
        ...
        <span slot="footer" class="dialog-footer">
          <el-button type="primary" class="lineclass" @click="popupisible = false">确 定</el-button>
        </span>
      </el-dialog>
      ...
      <el-dialog
        title="在线编辑"
        :visible.sync="onlineeds"
        width="70%"
        :before-close="handleClose">

        <div class="incoupt">
          <label>模板名称：</label>
          <el-input placeholder="请输入内容" v-model="taskName" clearable @keyup.enter.native="queryselect" name="clearInputValue"  size="small"></el-input>
          <el-button type="primary" class="bespoke" id="quet" @click="queryName">查询</el-button>
        </div>
        ...
        <el-table
          ...
          <el-table-column
            width="55">
            <template slot-scope="scope">
              <el-radio v-model="radioCur" :label="scope.row.id">&nbsp;</el-radio>
            </template>
          </el-table-column>
          <el-table-column
          align="center"
        ...
        <span slot="footer" class="dialog-footer">
          <el-button @click="onlineeds = false">取 消</el-button>
          <el-button type="primary" @click="onlineFn">确 定</el-button>
        </span>
      </el-dialog>
       <!-- 导入历史原始记录 -->
      <el-dialog
        ...
        <div class="inBoxs">
          <div class="incoupt2">
            <label>原始记录：</label>
            <el-input placeholder="请输入内容" v-model="originalname" clearable @keyup.enter.native="queryselect" name="clearInputValue"  size="small"></el-input>
          </div>
          <div class="incoupt2">
            <label>项目名称：</label>
            <el-input placeholder="请输入内容" v-model="projectName" clearable @keyup.enter.native="queryselect" name="clearInputValue"  size="small"></el-input>
          </div>
          <div class="incoupt2">
            <label>单位名称：</label>
            <el-input placeholder="请输入内容" v-model="unitname" clearable @keyup.enter.native="queryselect" name="clearInputValue"  size="small"></el-input>
          </div>
          <div class="incoupt2">
            <label>任务编号：</label>
          ...
          <el-button type="primary" class="bespoke" id="quet2" @click="historyquery()">查询</el-button>
        </div>
        <el-table
          ...
          <el-table-column
            width="55">
            <template slot-scope="scope">
              <el-radio v-model="radiohistoryCur" :label="scope.row.id">&nbsp;</el-radio>
            </template>
          </el-table-column>
          <el-table-column
          align="center"
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="historyFn">确 定</el-button>
        </span>
      </el-dialog>
  ...
  export default {
    ...
    data () {
      return {
        ...
        isnetfoot:false,
        //原始记录上传 新修改
        popupisible:false,//原始记录上传弹框
        singleTable:[],//原始记录上传数据
        numsing:0,//原始记录上  本地上传文件个数
        munlength:0,//原始记录上  本地上传记录次数，当次数=上传文件的个数时  弹出成功
        fileListmg:[],//存文件
        errstrs:[],//本地上传异常信息拼接
        onlineeds:false,//在线编辑弹框
        onlinetableData:[],//在线编辑原始记录模板数据
        bindingCurrentPage: 1, // 绑定模板列表-当前页
        bindingPageSize: 5, // 绑定模板列表-每页的条数
        bindingTotal:0,//绑定模板列表-总数
        taskName:'',//模板查询名称
        radioCur:'',//单选  在线编辑原始记录模板
        historyVisible:false,//导入历史数据弹出框
        historytableData:[],//导入历史原始记录模板数据
        radiohistoryCur:'',////单选  导入历史数据
        originalname:'',//原始记录：
        projectName:'',//项目名称：
        unitname:'',//单位名称：
        tasknumber:'',//任务编号：

        historyCurrentPage: 1, // 入历史数据模板列表-当前页
        historyPageSize: 5, // 入历史数据模板列表-每页的条数
        historyTotal:0,//入历史数据模板列表-总数
        ...
        //
        mlyTaskinfor:null,
        src:'static/OriginalRecord/record/record/record.html'//编辑原始记录模板路径
      }
    },
    components: {
      taskDetail,
      ...
      * */
      ...
      onClickXgdReport (row) {
        ...
        if (this.multi.length === 0) {
          ...
          if (this.ars == '') {
            this.$message.error({message: `请选择校核人员再提交!`, duration: 1500})
          } else {
            if (!that.check(obj.inspectids, obj.assignee)) {
              that.$message.error({message: `所选数据校核人员为必填项!`, duration: 1500})
              return
            }
            submission()
          }
      ...
      getMode () {
        ...
        // })
        this.report_type = '13'
        this.processInstanceIdName = 'JYYSCYSJL'
        this.yearDate()
        this.initProps()
        //this.initdata()
        // 看板页模块的表格,一些操作不处理
      ...
      // 新增方法
      clickOriginalrecord (row) { //原始记录上传
        this.fileList = []
        this.rowdata  = row
        ...
        //     let name = item.fileName+'.'+item.fileType
        //     console.log(name)
        //     this.fileList.push({name:name,fileId:item.fileId})
        //   });
        ...
        this.$api.lxtinitial.getOriginalOnlineId(taskContentId).then(res => {
          this.singleTable = res.data.data
        }).then(()=>{
          this.popupisible = true;
      ...
      handleRemove (file, fileList) {
        this.fileListmg = fileList
      },
      handleChange(file, fileList){
        this.fileListmg = fileList
      },
      handleExceed (files, fileList) {
        this.$message.warning(`每次最多上传100条数据！`)
      },
      submitUpload() {
        this.munlength = this.fileListmg.length
        this.numsing = 0;
        this.errstrs = [];
        this.$refs.upload.submit();
      },
      beforeUpload (file) {//上传按钮shamhchuan
        let myFlage = false;
        this.numsing ++;
        console.log(this.numsing)
        console.log(this.munlength)
        if(this.numsing == this.munlength){
          myFlage = true;
        }
        // docx,doc,xls,xlsx,pdf,jpg,png
        ...
        const extension7 = testmsg === 'png'
        const extension8 = testmsg === 'csv'
        // const isLt2M = file.size / 1024 / 1024 < 10
        if(!extension && !extension2 && !extension3 && !extension4 && !extension5 && !extension6 && !extension7 && !extension8) {
              this.$message({
                message: '上传文件只能是docx,doc,xls,xlsx,pdf,jpg,png,csv格式!',
                type: 'warning'
            });
        }else{
          this.numfile = 0
          ...
          let fd = new FormData();
          fd.append('taskContentId', this.onetaskContentId)  //报告id
          fd.append('taskId', this.onetaskid)  //任务id
          //fd.append('fileId', '')//文件id
          //fd.append('fileId', file.uid)//文件id
          fd.append('pathFolder', '')//文件路径
          fd.append('type', 5)
          fd.append('file', file)
          this.$api.lxtinitial.rogerup(fd).then(res => {
            let data = res.data
            if (data.respCode == 0) {
              if(myFlage){
                this.$alert(res.data.message, '信息提示', {
            ...
            if(res){
              if(this.errstrs.length == this.munlength){
                return false
              }
              let data = {
              ...
              this.clickOriginalrecord(this.rowdata);
              this.$api.lxtinitial.saveUploadAttachStatus(data).then(res => {})
            }
          }).catch(res => {
            this.errstrs.push(res.data.message)
          })
        }
        if(myFlage){
            setTimeout(()=>{
              if(this.errstrs.length > 0){
                this.$alert(this.errstrs.join(), '信息提示', {
                  dangerouslyUseHTMLString: true
                })
              }
            },2000)
      ...
      //   this.fileList =[]
      // }
      handleDelete(index,row){ //原始记录在线编辑-根据id删除原始记录信息
          this.$api.lxtinitial.deleteId(row.id).then(res => {
      ...
      onlineediting(isname){ //原始记录在线编辑
        let nameid ;
        if(isname){
          nameid = ''
        }else{
          nameid = this.taskName;
          this.bindingCurrentPage=1;
          this.bindingPageSize=5;
        }
        const data = {
            "name": nameid,  //模板名称
            "items_itme_name": '',  //检验项目名称
            "items_test_basis": '',  //检验依据
            "status": '',  //状态 =1 已绑定项目 =2 未绑定项目  全部传空
        ...
        this.$api.paramsConfig.originalrecordlist(data).then(res => {
          console.log(res)
            if (res.data.respCode == 0) {
                ...
                this.onlinetableData = data.list;
                this.bindingTotal = data.total
                this.onlineeds = true;
            }
        }).catch(res => {
            this.$alert(res.data.message, '信息提示', {
      ...
      queryName(){ //在线编辑查询
        this.onlineediting(false)
      },
      onlineFn(){ //在线编辑查询 确定按钮
        this.$api.lxtinitial.chooseRecordTemplate(this.onetaskContentId,this.radioCur).then(res => {
          if(res.data.ok == true){
            this.$alert('操作成功', '信息提示', {
                dangerouslyUseHTMLString: true
            });
            this.onlineeds = false;
          }
        }).then(()=>{
          this.clickOriginalrecord(this.rowdata);
        }).catch((err)=>{
      ...
      importhistory(isname){ // 导入历史原始记录
        ...
        let data ={
          "app": false,   //是否是来源App  true为app端 false为pc端
          "companyCheck": this.unitname,  //单位名称
          "itemsBasis": "",  //检验依据
          "itemsName": this.projectName,  //项目名称
          "name": this.originalname,  //原始记录
          "pageSize": this.historyPageSize,   //每页的数量
          "currentPage": this.historyCurrentPage,   //当前页
          "taskCode": this.tasknumber,  //任务号
        }
        this.$api.lxtinitial.getHistory(data).then(res => {
             if(res.data.ok == true){
      ...
      // 导入历史原始记录-分页器-设置每条的页数
      historyHandleSizeChange(val) {
        this.historyPageSize = val;
        this.importhistory(true)
      },
      // 导入历史原始记录-分页器-当前页发生改变时
      historyHandleCurrentChange(val) {
        this.historyCurrentPage = val;
        this.importhistory(true)
      },
      historyFn(){ //导入历史原始记录查询 确定按钮
        this.$api.lxtinitial.historyTemplate(this.onetaskContentId,this.radiohistoryCur).then(res => {
          if(res.data.ok == true){
            this.$alert(res.data.data, '信息提示', {
                dangerouslyUseHTMLString: true
            });
      ...
      historyquery(){ //导入历史原始记录查询
         this.importhistory(false)
      },
      // 原始记录上传 查看
      handleLook(index, row) {
        this.originalRecordForm = this.rowdata;
      ...
      handleEdit(index,row){ //编辑原始记录
        window.open(this.src+'?id='+row.id+'&taskContentId='+this.onetaskContentId, '_blank');

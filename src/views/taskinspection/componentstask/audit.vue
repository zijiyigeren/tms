    ...
    <div class="allot" :style="{
      ...
      <div class="allot_top" v-if="isComing != 0">
        ...
        <el-table ref="table" v-loading="isLoading" class="copyStyle"  v-if="isfoot"
            ...
            <el-table-column  label="报告编辑科室" align="center" :resizable="false"></el-table-column>
            <el-table-column  label="报告编辑人" align="center" :resizable="false"></el-table-column>
                ...
                <div>
                  ...
                  <el-table ref="Table2" border v-loading="loading2"
                    ...
                    <el-table-column prop="itemsFrederickItme" label="检验方法" align="center"
                                     :resizable="false"></el-table-column>
                    <el-table-column prop="testRoomName" label="检验科室" align="center"
                                     :resizable="false"></el-table-column>
                    <el-table-column prop="orgName" label="检验部科室主任" align="center" :resizable="false">
                      <template slot-scope="scope">
                        <el-select size="mini" style="width:80%;" disabled
                                   @change="currentSel(tableData2[scope.$index].id,scope.row,scope.$index)">
                          <el-option v-for="(item,i) in tableData2[scope.$index].zhuRenList" :key="i"
      ...
      * */
      ...
      dataSelect (val) {
        ...
        for (let i = 0; i < val.length; i++) {
          bainjike.push(this.findOfficeitem)
          bainjiren.push(this.finditingitem)
        }
      ...
      toogleExpand (row) {  // 第一个弹出层
        // this.findOfficeMode();  不需要调用
        this.carType = row.examineName
        this.multi = []
        this.loading2 = true
        let obj = {
          companyCheck: this.formLabelAlign.unit,
      ...
      getMode () {
        ...
        // })
        this.report_type = '10'
        this.processInstanceIdName = 'RWXXSH'
        this.yearDate()
        this.initProps()
        this.initdata()
        // 看板页模块的表格,一些操作不处理

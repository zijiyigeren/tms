              ...
              <template slot-scope="scope">
                ...
                <div>
                  <el-table ref="Table2" border v-loading="loading2"
                    ...
                    <el-table-column prop="assignee" label="主检" align="center" :resizable="false">
                      <template slot-scope="scope">
                        <el-select
                          size="mini"
                          style="width:80%;"
                          ...
                          <el-option v-for="(item,i) in tableData2[scope.$index].zhuJianList" :key="i" :label="item.loginName+'_'+item.name"
    ...
    methods: {
      ...
      * */
      ...
      let that = this
        ...
        let _dector = this.strs.split(',')   
        let _arr = []
        _dector.forEach(item => {
          if(item != '' && item != null){
            _arr.push(item)
          }
        });
        ...
        else if(this.multi.length != _arr.length){
          this.$message({
            type: 'error',
            message: '主检未填写!',
            duration: 1500
        ...
        this.$api.allot.findJobContent(obj).then(res => {
          ...
          this.tableData2.map(item => {
            ...
            if(item.needReport == 0){
             ...
             if(item.zhuJianList[0]){
              item.assignee = item.zhuJianList[0].loginName
            }
      ...
      getMode(){
        ...
        // })
        this.report_type = '2'
        this.processInstanceIdName ='KSZRFP'
        this.initProps();
          // this.initdata();
          // 看板页模块的表格,一些操作不处理

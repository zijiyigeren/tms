...
var KisBpmAssignmentPopupCtrl = ['$scope', '$http', function ($scope, $http) {
    ...
    //初始化
    $scope.init = function () {
        console.log($scope.property.value.assignment,'初始化assignment');
        if ($scope.property.value.assignment) {
            if ($scope.property.value.assignment.assignee) {
                $http({
                    method: 'GET',
                    ...
                    if (data.ok == true) {
                        ...
                        console.log($scope.selectedUserArr, '初始化selectedUserArr');
                    }
                }).error(function (data, status, headers, config) {
    ...
    $scope.getUserByRole = function () {
        var roleName = $scope.selectRole;
        var searchUrl = KISBPM.URL.getUser() + "?pageNo=1&pageSize=99999&roleName="+encodeURI(roleName);
        $http({
            method: 'GET',
            url: searchUrl,
        }).success(function (data, status, headers, config) {
            if (data.ok == true) {
                ...
                console.log($scope.searchListByRole, '获取角色下的用户成功');
                
            }
        }).error(function (data, status, headers, config) {
            console.log(data, '获取角色下的用户失败!');

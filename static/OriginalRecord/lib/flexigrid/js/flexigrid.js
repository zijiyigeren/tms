...
(function ($) {
    ...
    $.addFlex = function (t, p) {
        ...
        var g = {
                ...
                if (p.dataType == 'json') {
                    $.each(data.rows, function (i, row) {
                        ...
                        if (p.isChecked) {
                            ...
                            $(check).data("flexigrid.row", row);
                            $(check).attr("headid", "flexigridSelectHead");
                            $(td).append(check);
                            $(tr).append(td);
                            td = null;
                        }
            ...
            clearCheck: function () {
                $("input[type=checkbox]", g.hDiv).each(function () {
                    this.checked = false;
                });
                $("input[type=checkbox]", g.bDiv).each(function () {
                    this.checked = false;
                });
            },
            getCheckData: function () {
                var checkData = [];
                $('input:checkbox[headid=flexigridSelectHead]:checked', g.bDiv).each(function () {
                    var row = $(this).parents("tr").data("flexigrid.row");
                    if (row) {
                        checkData.push(row);
                    }
                });
            ...
            addRowProp: function () {
                $('tbody tr', g.bDiv).on('click', function (e) {
                    ...
                    if (obj.type == "checkbox") {
                        if ($("[headid=flexigridSelectHead]", t).length == $('input:checkbox[headid=flexigridSelectHead]:checked', t).length) {
                            $("input[type=checkbox]", $("#flexigridSelectHead", g.hDiv))[0].checked = true;
                        } else {
                            $("input[type=checkbox]", $("#flexigridSelectHead", g.hDiv)).removeAttr("checked");
                        }
                    };
        ...
        if (p.colModel) { //create model if any
            ...
            if (p.isChecked) {
                ...
                th.className = "fixedhead";
                th.tag = "flexiSelecColumn";
                th.align = "center";
                $(th).attr('width', "30px");
                $(th).attr("id", "flexigridSelectHead");
                $(th).attr('columnType', "checkboxColumn");
                $(tr).append(th);
            }
        ...
        $('thead tr:first th', g.hDiv).each(function () {
            ...
            if (!p.colmodel && $(this).attr("columnType") != "checkboxColumn") {
            ...
            if ($(this).attr("columnType") == "checkboxColumn") {
                var checkbox = $("<input type='checkbox' />");
                $(thdiv).append(checkbox);
                checkbox.on("click", function () {
                    var checked = this.checked;
                    $("[headid=flexigridSelectHead]", t).each(function () {
        ...
        if (p.usepager) {
            ...
            $(g.bDiv).after(g.pDiv);
            var html = ' <div class="pGroup"> <div class="pFirst pButton"><span></span></div><div class="pPrev pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup" style="margin-top: -2px;"><span class="pcontrol">' + p.pagetext + ' <input type="text" size="4" value="1" /> ' + p.outof + ' <span> 1 </span></span></div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pNext pButton"><span></span></div><div class="pLast pButton"><span></span></div> </div> <div class="btnseparator"></div> ';
    ...
    $.fn.flexSearch = function (p) { // function to search grid
        return this.each(function () { if (this.grid) this.grid.doSearch(); });
    }; //end flexSearch

    $.fn.clearCheck = function (p) { // function to clearCheck grid
        return this.each(function () {
            if (this.grid) this.grid.clearCheck();
        });
    }; //end clearCheck

    $.fn.getCheckData = function (p) { // function to getCheckData grid
        var checkData;
        this.each(function () {
            if (this.grid) checkData = this.grid.getCheckData();
        })
        return checkData;
    }; //end getCheckData
})(jQuery);

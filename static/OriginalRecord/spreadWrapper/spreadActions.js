    ...
    var SetAutoFormulaAction = (function (_super) {
        ...
        SetAutoFormulaAction.prototype._setFormulaForSingleRange = function (range, formulaName) {
            ...
            if ((range.colCount === 1 && range.rowCount === 1) || rangeIsSpan) {
                ...
                var formulaRange = self.getFormulaRange(range);
                for (var i = 0; i < range.rowCount; i++) {
                    if (self._noNeedSetFormula(range.row + i, range.col)) {
                        break;
                    }
                    formulaRange.row = formulaRange.row + i;
                    var formulaString = rangeToFormula(formulaRange, range.row + i, range.col, GC.Spread.Sheets.CalcEngine.RangeReferenceRelative.allRelative, self._spread.options.referenceStyle === 1/* useR1C1 */);
                    var formula = "=" + formulaName + "(" + formulaString + ")";
                    sheet.setFormula(range.row + i, range.col, formula);
                    formulaRange.row = formulaRange.row - i;
                }
            } else if (range.rowCount === 1) {
                var formulaRange = self.getFormulaRange(range);
            for (var i = 0; i < range.colCount; i++) {
                    if (self._noNeedSetFormula(range.row, range.col + i)) {
                        break;
                    }
                    formulaRange.col = formulaRange.col + i;
                    var formulaString = rangeToFormula(formulaRange, range.row, range.col + i, GC.Spread.Sheets.CalcEngine.RangeReferenceRelative.allRelative, self._spread.options.referenceStyle === 1/* useR1C1 */);
                    var formula = "=" + formulaName + "(" + formulaString + ")";
                    sheet.setFormula(range.row, range.col + i, formula);
                    formulaRange.col = formulaRange.col - i;
                }
            } else {
